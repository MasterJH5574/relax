...
bool HasReshapePattern(const PrimFunc& func) {
  class ReshapeDetector : public StmtVisitor {
   ...
   private:
    ...
    void VisitStmt_(const ForNode* loop) final {
      ...
      // To detect the reshape pattern, we require each For to have either another For or a
      // BlockRealize as body.
    ...
    void VisitStmt_(const BlockRealizeNode* block_realize) final {
      // Step 1. Constructing the mapping from block iterators to iterator binding values. The
      // mapping will be used in the substitution of the flattened buffer access index.
      ...
      for (int i = 0; i < n_iter; ++i) {
        ...
        var_map.Set(block_iter[i]->var, iter_values[i]);
      }

      // Step 2. Get the load/store pattern of the block body.
      // To detect the reshape pattern, we require the block body to be a BufferStore, which has a
      // BufferLoad as value.
      ...
      // Further, we require the buffer being stored and being loaded to match the parameter of the
      // PrimFunc, namely `dst_buffer_` and `src_buffer_`.
      ...
      // Step 4. Substitute the block iterators in the flattened index with loop variables, and
      // check if we can prove their equality.
      src_idx = tir::Substitute(std::move(src_idx), var_map);
      dst_idx = tir::Substitute(std::move(dst_idx), var_map);
  ...
  Optional<Buffer> src_buffer = func->buffer_map.Get(func->params[0]);
  Optional<Buffer> dst_buffer = func->buffer_map.Get(func->params[1]);
  ...
  // To detect the reshape pattern, we require each For to have either another For or a
  // BlockRealize as body.
  const Stmt& block_body = Downcast<BlockRealize>(func->body)->block->body;
  if (!block_body->IsInstance<ForNode>()) {
    return false;
  }
  return ReshapeDetector::Detect(src_buffer.value(), dst_buffer.value(), block_body);
}
