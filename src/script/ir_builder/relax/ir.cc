...
void DataflowBlockOutput(const Array<tvm::relax::Var>& vars) {
  ...
  // single round of visit, we adoopt a two-round visit to deal with the construction of
  // dataflow block.
  ...
  // - Therefore, when visitng dataflow output for the first time, we collect the names of the
...
BlockFrame CheckBlockFrameExistAndUnended() {
  ...
  // wrapped by `with R.dataflow()`), it is possible that there is no exisitng BlockFrame. In this
...
tvm::relax::Var EmitMatchShape(const tvm::relax::Expr& value, const Array<PrimExpr>& pattern,
  ...
  if (!block_frame->output_var_names.defined()) {
    return block_builder->EmitMatchShape(value, pattern, var_name);
  } else {
    // If we directly emit the expression by providing the desired variable name, the block builder
    // will automatically rename the variable whatever. Therefore, we manually create the variable
    // with correct fields and the binding. And finally emit the manually created binding.
...
void EmitMatchShapeWithoutVar(const tvm::relax::Expr& value, const Array<PrimExpr>& pattern) {
  CheckBlockFrameExistAndUnended();
  tvm::relax::BlockBuilder block_builder = GetBlockBuilder();
