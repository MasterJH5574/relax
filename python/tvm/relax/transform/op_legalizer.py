...
def _concat(bb: BlockBuilder, call: Call):
    ...
    fields = t.fields if isinstance(t, Tuple) else [TupleGetItem(t, i) for i in range(n_field)]
    return bb.call_te(
...
def _nn_adaptive_max_pool2d(bb: BlockBuilder, call: Call):
    ...
    if call.attrs.output_size == None:
...
class OperatorLegalizer(PyExprMutator):
    ...
    def _convert_op(self, call: Call) -> Call:
        ...
        if call.op.name != "relax.call_tir":
            logging.info(f"No legalization func for {call.op} is found.")
        return call
